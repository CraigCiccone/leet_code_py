...
# integer array [index1, index2] of length 2. The tests are generated such that there is exactly one solution. You may
...
class Solution:
    def twoSum(self, numbers: List[int], target: int) -> List[int]:
        ...
        for idx in range(len(numbers)):
            compliment = target - numbers[idx]

            if numbers[idx] in compliments:
                return [compliments[numbers[idx]] + 1, idx + 1]

            compliments[compliment] = idx
