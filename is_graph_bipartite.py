        ...
        for node_idx in range(len(graph)):
            if colors[node_idx] == 0:
                ...
                while q:
                    ...
                    for neighbor in graph[cur_node_idx]:
                        if colors[neighbor] == colors[cur_node_idx]:
                            return False
                        elif colors[neighbor] == 0:
                            q.append(neighbor)
                            colors[neighbor] = -colors[cur_node_idx]
...
if __name__ == "__main__":
    solution = Solution()
    print(solution.is_bipartite([[1, 3], [0, 2], [1, 3], [0, 2]]))
    print(solution.is_bipartite([[1, 2, 3], [0, 2], [0, 1, 3], [0, 2]]))
